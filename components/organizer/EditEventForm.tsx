...
export function EditEventForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        let imageUrl: string | undefined = event.imageUrl ?? undefined;
        if (fileToUpload) {
        ...
        await updateEvent(event.id, { ...form, customFields, imageUrl });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="totalSeats">
            Total Seats * <span className="text-muted-foreground font-normal">(current registrations: {event.registrationCount})</span>
          </Label>
          <Input
            id="totalSeats"
            type="number"
